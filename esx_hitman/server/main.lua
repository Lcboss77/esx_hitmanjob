...
AddEventHandler('esx_hitmanjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'police' then
		print(('esx_hitmanjob: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
...
AddEventHandler('esx_hitmanjob:handcuff', function(target)
	...
	if xPlayer.job.name == 'hitman' then
		...
		print(('esx_hitmanjob: %s attempted to handcuff a player (not hitman)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_hitmanjob:drag', function(target)
	...
	if xPlayer.job.name == 'hitman' then
		TriggerClientEvent('esx_hitmanjob:drag', target, source)
	else
		print(('esx_hitmanjob: %s attempted to drag (not hitman)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_hitmanjob:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'hitman' then
		...
		print(('esx_hitmanjob: %s attempted to put in vehicle (not hitman)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_hitmanjob:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'hitman' then
		...
		print(('esx_hitmanjob: %s attempted to drag out from vehicle (not hitman)!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_hitmanjob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_hitmanjob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_hitmanjob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_hitmanjob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_hitmanjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_hitmanjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_hitmanjob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_hitmanjob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'hitman' then
			Citizen.Wait(5000)
			TriggerClientEvent('esx_hitmanjob:updateBlip', -1)
		end
	end
end)

RegisterServerEvent('esx_hitmanjob:spawned')
AddEventHandler('esx_hitmanjob:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_hitmanjob:forceBlip')
AddEventHandler('esx_hitmanjob:forceBlip', function()
	TriggerClientEvent('esx_hitmanjob:updateBlip', -1)
end)
...
RegisterServerEvent('esx_hitmanjob:message')
AddEventHandler('esx_hitmanjob:message', function(target, msg)
