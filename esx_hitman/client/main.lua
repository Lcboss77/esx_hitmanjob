local PlayerData, CurrentActionData, handcuffTimer, dragStatus, blipsBM, currentTask, spawnedVehicles = {}, {}, {}, {}, {}, {}, {}
...
function OpenCloakroomMenu()
	...
	if grade == 'soldato' then
		table.insert(elements, {label = _U('bm_wear'), value = 'rookie_wear'})
	elseif grade == 'capo' then
		table.insert(elements, {label = _U('bm_wear'), value = 'security_wear'})
	elseif grade == 'transporter' then
		table.insert(elements, {label = _U('bm_wear'), value = 'transporter_wear'})
	elseif grade == 'rd' then
		table.insert(elements, {label = _U('bm_wear'), value = 'rd_wear'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if
			data.current.value == 'rookie_wear' or
			data.current.value == 'security_wear' or
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				shopCoords = Config.hitmanStations[station].Vehicles[partNum].InsideShop
			...
			elseif type == 'helicopter' then
				shopCoords = Config.hitmanStations[station].Helicopters[partNum].InsideShop
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	local spawnPoints = Config.hitmanStations[station][part][partNum].SpawnPoints
...
RegisterNetEvent('esx_hitmanjob:handcuff')
AddEventHandler('esx_hitmanjob:handcuff', function()
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'hitman' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'hitman_actions') then
			if Config.MaxInService == -1 then
...
function createBlip(id)
	...
	if not DoesBlipExist(blip) then -- Add blip and create head display on player
		...
		table.insert(blipsBM, blip) -- add blip to array so we can remove it later
	end
end

RegisterNetEvent('esx_hitmanjob:updateBlip')
AddEventHandler('esx_hitmanjob:updateBlip', function()

	-- Refresh all blips
	for k, existingBlip in pairs(blipsBM) do
